...
![](/post/tutorial/figs/round_robin.png)
...
Another communication strategy is to pair each worker with a
neighbor. 

- Each site $i$ sends $E(l_i + r_i)$ and $E(l_i - r_i)$ to its
  neighbor workers. 
- In a synchronization step, all workers sum up what they got from
  their neighbors.
- Master then queries each worker which responds the sum. 
- Master sums all of these together.
